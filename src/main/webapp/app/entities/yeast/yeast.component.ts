...
export class YeastComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.yeastService
                ...
                .subscribe(
                    (res: HttpResponse<IYeast[]>) => this.paginateYeasts(res.body, res.headers),
    ...
    registerChangeInYeasts() {
        this.eventSubscriber = this.eventManager.subscribe('yeastListModification', response => this.reset());
    }
    ...
    protected paginateYeasts(data: IYeast[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.yeasts.push(data[i]);
        }
    }
