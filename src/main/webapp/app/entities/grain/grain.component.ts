...
export class GrainComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.grainService
                ...
                .subscribe(
                    (res: HttpResponse<IGrain[]>) => this.paginateGrains(res.body, res.headers),
    ...
    registerChangeInGrains() {
        this.eventSubscriber = this.eventManager.subscribe('grainListModification', response => this.reset());
    }
    ...
    protected paginateGrains(data: IGrain[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.grains.push(data[i]);
        }
    }
