...
public class BatchResourceIntTest {
     ...
     *
     * @see com.therobotcarlson.distilled.repository.search.BatchSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void searchBatch() throws Exception {
        // Initialize the database
        batchRepository.saveAndFlush(batch);
        when(mockBatchSearchRepository.search(queryStringQuery("id:" + batch.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(batch), PageRequest.of(0, 1), 1));
        // Search the batch
